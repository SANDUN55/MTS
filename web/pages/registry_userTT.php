...
<div class="container">
    <nav class="" aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="active breadcrumb-item" aria-current="page">Timetable Managers</li>
        </ol>
    </nav>
	<p id="success"></p>
        <div class="table-wrapper">
            <div class="table-title">
                <div class="row">
                    <div class="col-sm-8">
                        <h2><i class="pe-7s-user"></i> <b>Timetable Manager Registry</b></h2>
					</div>
                </div>
            </div>
<?php
...
//FOR TWO CHAIRS
//$result = mysqli_query($conn, "SELECT CONCAT( b_no, '-' , m_code ) as val FROM `batchmodule` WHERE cordi IN (SELECT st_id FROM staff WHERE username='$chair' ) OR cordi2 IN (SELECT st_id FROM staff WHERE username='$chair' ) ; ");
//FOR CONVENOR
...
//
$result = mysqli_query($conn," SELECT b_no, m.m_code, CONCAT( b_no, '-' , m.m_code ) as val, CONCAT (m_name, ' ', m_phase) as mod_nm,
cordi, CONCAT (c1.t_nm, ' ', c1.firstname , ' ', c1.surname) as cordi_nm , c1.dep_code, c1.div_nm,
ttmng1 , CONCAT (t1.t_nm, ' ', t1.firstname , ' ', t1.surname, '<hr>', t1.div_nm) as ttmng1_nm
FROM batchmodule 
LEFT JOIN staff c1 ON cordi=c1.st_id
LEFT JOIN staff t1 ON ttmng1=t1.st_id
LEFT JOIN module m ON batchmodule.m_code = m.m_code
WHERE ttprogress <= 3
ORDER BY b_no DESC , m_phase, m_name ;");
?>
            <table class="table table-striped table-hover">
                ...
                <tr>
                    ...
                    <th  align="center">CONVENOR</th>
                  <!--  <th  align="center">CHAIR</th>-->
                    <th align="center">TIMETABLE MANAGER</th>
                    <th> </th>
                 <!--   <th>TT MNG2</th>
                    <th> </th>
                    <th>TT MNG3</th>
...
while($row = mysqli_fetch_array($result)) {
    ...
    ?>
				<tr id="<?php echo $row["b_no"]; ?>"><!-- echo $row["b_no"]; -->
					...
					<td><?php echo $row["cordi_nm"]."<hr>".$row["div_nm"]; ?></td>
                  <!--  <td><?php /*echo $row["cordi2_nm"]; */?></td>-->
                    ...
                    <td align="center"> <?php if($editOn == 1) {  ?>
                        <a href="#editTTmngModal" class="edit" data-toggle="modal">
                            <i class="pe-7s-pen icon-gradient bg-sunny-morning updatett" data-toggle="tooltip"
                               data-id1="<?php echo $row["m_code"].',T1'; ?>"
                               data-bn="<?php echo $row["b_no"]; ?>"
                               data-mc="<?php echo $row["mod_nm"]; ?>"
                               data-dp="<?php echo $row["dep_code"]; ?>"
                               title="Edit"></i>
                        </a>
                         <a href="#deleteTTmngModal" class="delete" data-id="<?php echo $val; ?>" data-toggle="modal">
        ...
        <div class="modal-content">
            <form id="update_form">
                ...
                <div class="modal-body">
                    ...
                    <input type="hidden" id="dp_u" name="dp_u" class="form-control" required>
                    ...
                    <script>
                        $("#editTTmngModal").on("show.bs.modal",function(){
                            var dep = document.getElementById("dp_u").value;
                            $.ajax({
                                type: "POST",
                                url: "assets/scripts/backend/get_val.php",
                                data:'fid=' +  9  +  '&fval=' + dep,
                                success: function(data){
                                    $("#selectNonAcademic").html(data);
                                }
                            });
                        });
                    </script>
                    <div class="form-group">
                        <label>Timetable Manager</label>
                            <select name="selectNonAcademic" id="selectNonAcademic" required class="form-control"></select>

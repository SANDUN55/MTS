...
<?php //include 'assets/scripts/backend/database.php'; ?>
                    ...
                    if(isset($_POST['confimMod']) ) {
                       ...
                       // $labconn = mysqli_connect('172.18.2.8', 'syslabreader', 'jBGsQj8D4', 'labshedule');
                        ...
                        echo "<script> alert('Data Added , Please check the report.'); </script>";
                    }
                    ?>
<div class="container">
    <nav class="" aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="active breadcrumb-item" aria-current="page">Reservation Details</li>
        </ol>
    </nav>
        <div class="table-wrapper">
            <div class="table-title">
                <div class="row">
                    <div class="col-sm-8">
                        <img  src="assets/images/maintenance1.png"  >
                        <h2><i class="pe-7s-bookmarks"></i> <b>Reservation Details</b></h2>
            ...
            <form class="needs-validation" novalidate id="user_form">
                <div class="position-relative form-group">
                 ...
                 <button class="mb-2 btn btn-warning" name="reserveDetails" value="0"><i class="pe-7s-cross" data-toggle="tooltip">Display List</i></button>
            </form>
              <?php
          /*      $sql = "SELECT b_no, batchmodule.m_code, CONCAT(m.m_name, ' ', m.m_phase) AS modnm
            ...
            ( cordi IN (SELECT st_id FROM staff WHERE username='$user') OR ttmng1 IN (SELECT st_id FROM staff WHERE username='$user')) ORDER BY b_no DESC; ";
          // echo $sql;
            ...
            /*if($row = mysqli_fetch_array($result)) {
              ...
              $selectedBatchMo = $_REQUEST["selectBatchMo"] ?? '';
              $str = explode("-", $selectedBatchMo, 2);
              $hasValidBatchModule = count($str) === 2 && trim($str[0]) !== '' && trim($str[1]) !== '';
              if ($hasValidBatchModule) {
              $bno = (int) trim($str[0]);
              $mcode = mysqli_real_escape_string($conn, trim($str[1]));

            ?>
            <h2>Batch <?php echo $bno . ' - ' . $mcode;  ?></h2>
           <!-- <div align="right"> <a href="#confirmModuleReservations" class="confirmRes" data-toggle="modal"
            ...
            <?php
            $get_classtimes = "SELECT class_id, class_topic_id, Date(class_start) AS dt, TIME(class_start) AS st, TIME(class_end) AS en, c.lab_code,  lab_nm, class_topic,
                                res_code, class_remark
            FROM  classschedules c
            LEFT JOIN lab l ON c.lab_code = l.lab_code
            JOIN classtopics
            WHERE class_topic_id =  topic_id
            AND b_no = $bno AND TRIM(m_code) = TRIM('$mcode') AND class_status = 1 ORDER BY  class_start" ;
            $result = mysqli_query($conn, $get_classtimes); ?>
            <table class="table table-striped table-hover">
                ...
                <tr>
                    ...
                    <th  align="center">RES STATUS</th>
                    <th  align="center">LAB STATUS</th>
                    <th></th>
                </tr>
                </thead>
                <tbody>
            <?php
                ...
                if ($result === false) {
                    echo '<tr><td colspan="7" class="text-danger">Unable to load reservation details: ' . htmlspecialchars(mysqli_error($conn)) . '</td></tr>';
                } else {
                while($row = mysqli_fetch_assoc($result)){
                $classDt = $row['dt']; $classSt = $row['st']; $classEn = $row['en']; $lab = $row['lab_code']; $reason = $row['class_topic']; $classID  = $row['class_id'];
                $labnm = $row['lab_nm']; $rescode = $row['res_code'];
                ...
                ?>
                 <tr id="<?php echo $row["class_id"]; ?>">
                        ...
                        <td width="15%"><?php echo $classSt . ' - ' . $classEn; ?></td>
                         <td width="30%"><?php echo $reason; ?></td>
                        <td width="15%"><?php echo $labnm; if($crem == 'http') echo 'online'; ?></td>
                        <td width="15%"><?php // echo $rescode;
                            if($crem == 'http') echo 'NA';
                            elseif($rescode == 0) {
                                echo 'Not Reserved';
                                echo "<i class='fa fa-fw' aria-hidden='true' ></i> ";
                            }
                            elseif($rescode >= 1 ){
                                echo 'Reserved ';
                                echo "<i class='fa fa-fw' aria-hidden='true'></i>";
                        ...
                        <td width="15%"><?php //echo $lab_availability;
                                if($crem == 'http') echo 'NA';
                                else {
                                    if($lab_availability == 'F') {
                                        echo '';
                                    }
                                    elseif($lab_availability == 'T') {
                                        echo 'Available';
                                    }
                                } ?></td>

...
$query = " SELECT CONCAT (class_id, '-', class_topic_id) as class_reserve_id, class_topic, class_start, class_end, class_group, CONCAT(t_nm, '. ', firstname, ' ', surname, ' ') AS stVAL , a_name, a_type, lab_nm FROM `classschedules`
...
ORDER BY `class_id`;";
$query .= "SELECT CONCAT (class_id, '-', class_topic_id) as class_reserve_id, class_topic, class_start, class_end, class_group, div_nm, a_name, a_type, lab_nm FROM `classschedules`
 JOIN classtopics ON class_topic_id = topic_id
    ...
    do {
        if ($result = mysqli_store_result($conn)) {
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                $TitleDescription = stripslashes( $classGroup) . ' : ' . $row["stVAL"].$row["div_nm"].' : '.$row["lab_nm"];
